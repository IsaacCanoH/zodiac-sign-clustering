...
def export_hierarchical_run(run):
    """Serialize a Hierarchical Clustering run together with its dataset identity."""
    return {
        'type': MODEL_TYPE,
...
def import_hierarchical_run(dataset, data, *, allow_compatible=False):
    """Validate and restore a Hierarchical Clustering run for a dataset."""
    require_mapping(data)
    if data.get('type') != MODEL_TYPE:
        raise ValueError('El archivo no corresponde a un modelo Jerárquico.')
    ...
    selected_columns = validate_selected_columns(
        dataset, data.get('selected_columns')
    )
    sample_count = require_integer(data.get('sample_count'), 'sample_count', 2)
    cluster_count = require_integer(
        data.get('cluster_count'), 'cluster_count', 2
    )
    n_clusters = require_integer(
        data.get('n_clusters', cluster_count), 'n_clusters', 2
    )
    if cluster_count >= sample_count:
        raise ValueError(
            'El modelo Jerárquico requiere menos clusters que registros.'
        )
    ...
    # Validate linkage and affinity
    linkage = data.get('linkage', 'ward')
    if linkage not in ('ward', 'complete', 'average', 'single'):
        raise ValueError('El método de enlace no es válido.')
    affinity = data.get('affinity', 'euclidean')
    if affinity not in ('euclidean', 'manhattan', 'cosine'):
        raise ValueError('La métrica de distancia no es válida.')
    if linkage == 'ward' and affinity != 'euclidean':
        raise ValueError('El método Ward requiere distancia Euclidiana.')
    scaling_method = data.get('scaling_method', 'standard')
    if scaling_method not in ('standard', 'minmax', 'none'):
        raise ValueError('El método de escalado no es válido.')
    ...
    if silhouette_sample_count > sample_count:
        raise ValueError(
            'La muestra de silueta supera los registros del modelo.'
        )
    ...
    with transaction.atomic():
        return HierarchicalRun.objects.create(
            ...
            version=require_integer(
                data.get('model_version', 1), 'model_version', 1
            ),
